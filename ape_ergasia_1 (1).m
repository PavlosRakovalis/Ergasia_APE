...
$$$$$$$$$$$$ Βήμα 1: Υπολογισμός συνολικού υπερπαραγούμενου ζεστού νερόυ και συνεπώς και νέου όγκου διεποχιακής αποθήκευσης.
...
%%%%%%% Βήμα 2: Υπολογισμός ογκου νερου που περισέυει μετά την κάλυψη των αναγκών του δεκεμβριου 
% και υπολογσιμός του ποσού του διοξειδίου του ανθρακα που θα εξοικονωμήσουμε 
...
% απο εδω και κάτω πρέπει να ελεχτούν 

remaining_excess_energy = total_excess - L(1);
% Έλεγχος αν η υπερπαραγωγή καλύπτει τις ανάγκες του Ιανουαρίου
if total_excess >= L(1)
    fprintf('\nΗ υπερπαραγωγή καλύπτει πλήρως τις ανάγκες του Ιανουαρίου.\n');
    fprintf('Περίσσεια ενέργειας μετά την κάλυψη Ιανουαρίου: %.2f kWh\n', remaining_excess_energy);
    
    % Υπολογισμός όγκου δεξαμενής για κάλυψη Ιανουαρίου
    Vtank_january = L(1) / (p * Cp * (45 - Tcw(1))) * 1000; % σε λίτρα
    fprintf('Όγκος δεξαμενής για κάλυψη Ιανουαρίου: %.2f λίτρα\n', Vtank_january);
    
    % Υπολογισμός εξοικονόμησης CO2 από την κάλυψη Ιανουαρίου
    E_fuel_saved = L(1) / h_boiler;
    ...
    fprintf('Υπερπαραγωγή: %.2f kWh, Ανάγκες Ιανουαρίου: %.2f kWh\n', total_excess, L(1));
    fprintf('Έλλειμμα που πρέπει να καλυφθεί από συμβατικές πηγές: %.2f kWh\n', L(1) - total_excess);
    
    % Υπολογισμός όγκου δεξαμενής για αποθήκευση όλης της υπερπαραγωγής
    Vtank_january = total_excess / (p * Cp * (45 - Tcw(1))) * 1000; % σε λίτρα
    fprintf('Όγκος δεξαμενής για αποθήκευση διαθέσιμης υπερπαραγωγής: %.2f λίτρα\n', Vtank_january);
